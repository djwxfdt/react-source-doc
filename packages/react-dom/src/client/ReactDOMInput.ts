...
import { ToStringValue, toString, getToStringValue } from "./ToStringValue";
...
export function getHostProps(element: Element, props: Record<string, any>) {
  const node = ((element as any) as InputWithWrapperState);
  const checked = props.checked;
  ...
  if (value != null) {
    if (type === 'number') {
      if (
        ...
        node.value = toString((value as any));
      }
    } else if (node.value !== toString((value as any))) {
      node.value = toString((value as any));
    }
...
function updateNamedCousins(rootNode: any, props: any) {
